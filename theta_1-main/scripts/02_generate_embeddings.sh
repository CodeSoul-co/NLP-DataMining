...
EMBEDDING_MODELS_DIR=${ETM_EMBEDDING_MODELS_DIR:-$BASE_DIR/embedding_models}
EMBEDDING_CODE_DIR=${ETM_EMBEDDING_CODE_DIR:-$BASE_DIR/embedding}
...
if [ -z "$MODEL_PATH" ]; then
    case $MODEL_SIZE in
        0.6B) MODEL_PATH="$EMBEDDING_MODELS_DIR/qwen3_embedding_0.6B" ;;
        4B)   MODEL_PATH="$EMBEDDING_MODELS_DIR/qwen3_embedding_4B" ;;
        8B)   MODEL_PATH="$EMBEDDING_MODELS_DIR/qwen3_embedding_8B" ;;
        *)    echo "Error: Unknown model size '$MODEL_SIZE'"; exit 1 ;;
    esac
    # Fallback to legacy path if embedding_models path doesn't exist
    if [ ! -d "$MODEL_PATH" ]; then
        LEGACY_PATH="$BASE_DIR/qwen3_embedding_${MODEL_SIZE}"
...
if [ -n "$EXP_DIR" ]; then
    CMD="$CMD --output_dir $EXP_DIR/embeddings"
fi

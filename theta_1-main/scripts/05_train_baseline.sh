...
# Base directories - support environment variables for flexible deployment
BASE_DIR=${ETM_BASE_DIR:-/root/autodl-tmp}
RESULT_DIR=${ETM_RESULT_DIR:-$BASE_DIR/result}
ETM_CODE_DIR=${ETM_CODE_DIR:-$BASE_DIR/ETM}
...
EXP_MANAGER="$ETM_CODE_DIR/experiment_manager.py"

# Auto-select data_exp if not provided
if [ -z "$DATA_EXP" ]; then
    BASELINE_DATA_DIR="$RESULT_DIR/baseline/$DATASET/data"
    if [ -d "$BASELINE_DATA_DIR" ]; then
        # Check if models include CTM (which requires SBERT)
        NEED_SBERT=false
        if echo "$MODELS" | grep -qE '(^|,)ctm(,|$)'; then
            NEED_SBERT=true
        fi
        
        # Find the latest experiment that has required files
        for EXP_DIR in $(ls -dt "$BASELINE_DATA_DIR"/exp_* 2>/dev/null); do
            HAS_BOW=false
            HAS_SBERT=false
            
            # Check for BOW
            if [ -f "$EXP_DIR/bow_matrix.npy" ] || [ -f "$EXP_DIR/bow/bow_matrix.npy" ]; then
                HAS_BOW=true
            fi
            
            # Check for SBERT embeddings
            if [ -f "$EXP_DIR/sbert_embeddings.npy" ] || [ -f "$EXP_DIR/embeddings/sbert_embeddings.npy" ]; then
                HAS_SBERT=true
            fi
            
            # Select this experiment if it meets requirements
            if [ "$HAS_BOW" = "true" ]; then
                if [ "$NEED_SBERT" = "false" ] || [ "$HAS_SBERT" = "true" ]; then
                    DATA_EXP=$(basename "$EXP_DIR")
                    echo "[INFO] Auto-selected data experiment: $DATA_EXP"
                    if [ "$NEED_SBERT" = "true" ] && [ "$HAS_SBERT" = "true" ]; then
                        echo "[INFO] SBERT embeddings found (required for CTM)"
                    fi
                    break
                else
                    echo "[WARN] Skipping $EXP_DIR: missing SBERT embeddings (required for CTM)"
                fi
            fi
        done
        
        # Fallback to latest if no suitable experiment found
        if [ -z "$DATA_EXP" ]; then
            LATEST_EXP=$(ls -dt "$BASELINE_DATA_DIR"/exp_* 2>/dev/null | head -1)
            if [ -n "$LATEST_EXP" ]; then
                DATA_EXP=$(basename "$LATEST_EXP")
                echo "[WARN] No experiment with all required files found, using latest: $DATA_EXP"
            fi
        fi
    fi
fi
...
if [ -n "$DATA_EXP" ]; then
    DATA_EXP_DIR="$RESULT_DIR/baseline/$DATASET/data/$DATA_EXP"
...
CONFIG_SCRIPT="$ETM_CODE_DIR/models_config/model_config.py"

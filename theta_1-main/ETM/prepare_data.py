    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(texts), batch_size), desc="Generating embeddings"):
            ...
            if hasattr(outputs, 'last_hidden_state'):
                batch_emb = outputs.last_hidden_state[:, 0, :].float().cpu().numpy()
            else:
                batch_emb = outputs[0][:, 0, :].float().cpu().numpy()
...
def generate_sbert_embeddings(texts: List[str], output_dir: Path, batch_size: int = 32) -> np.ndarray:
    ...
    # Support environment variable ETM_EMBEDDING_MODELS_DIR for flexible deployment
    embedding_models_dir = os.environ.get("ETM_EMBEDDING_MODELS_DIR", "/root/autodl-tmp/embedding_models")
    print(f"  ETM_EMBEDDING_MODELS_DIR: {embedding_models_dir}")
    
    local_sbert_paths = [
        '/mnt/sbert',  # DLC mount path (embedding_models/sbert/all-MiniLM-L6-v2 -> /mnt/sbert)
        f'{embedding_models_dir}/sbert/all-MiniLM-L6-v2',
        f'{embedding_models_dir}/all-MiniLM-L6-v2',
        f'{embedding_models_dir}/sbert',
        '/mnt/embedding_models/sbert/all-MiniLM-L6-v2',
        '/mnt/embedding_models/all-MiniLM-L6-v2',
        '/mnt/embedding_models/sbert',
    ...
    for path in local_sbert_paths:
        print(f"  Checking: {path} -> {'EXISTS' if Path(path).exists() else 'NOT FOUND'}")
        if Path(path).exists():
            # Verify it has model files (config.json or pytorch_model.bin)
            p = Path(path)
            has_config = (p / 'config.json').exists() or (p / 'config_sentence_transformers.json').exists()
            has_model = (p / 'pytorch_model.bin').exists() or (p / 'model.safetensors').exists()
            if has_config or has_model:
                local_sbert_path = path
                print(f"  -> Valid model found!")
                break
            else:
    ...
    if local_sbert_path:
        ...
        print(f"  [WARNING] No local SBERT model found, attempting online download...")

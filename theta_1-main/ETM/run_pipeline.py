...
def parse_args():
    ...
    # Job ID for cloud runs (data isolation)
    parser.add_argument('--job_id', type=str, default=None,
                        help='Job ID for cloud runs. If set, data is read from data/{job_id}/ and results are saved to result/{job_id}/')
...
def check_theta_data_files(dataset: str, model_size: str, mode: str, data_exp_dir: str = '') -> Dict[str, Any]:
    ...
    if data_exp_dir:
        ...
        emb_candidates = [
            ...
            exp_path / 'embeddings' / mode / f'{dataset}_{mode}_embeddings.npy',  # e.g., embeddings/zero_shot/edu_data_zero_shot_embeddings.npy
        ]
        # Also search for any .npy file in embeddings directory recursively
        emb_dir = exp_path / 'embeddings'
        if emb_dir.exists():
            for npy_file in emb_dir.rglob('*.npy'):
                if npy_file not in emb_candidates:
                    emb_candidates.append(npy_file)
...
def _is_complete_theta_exp(exp_dir: Path) -> bool:
    ...
    bow_exists = (exp_dir / 'bow' / 'bow_matrix.npy').exists()
    # Check for any .npy file in embeddings directory (including subdirectories)
    emb_dir = exp_dir / 'embeddings'
    emb_exists = False
    if emb_dir.exists():
        for _ in emb_dir.rglob('*.npy'):
            emb_exists = True
            break
    return bow_exists and emb_exists
...
def main():
    ...
    # Job ID mode: override data and result directories
    if args.job_id:
        global DATA_DIR, RESULT_DIR
        from config import BASE_DIR
        DATA_DIR = BASE_DIR / "data" / args.job_id
        RESULT_DIR = BASE_DIR / "result" / args.job_id
        print(f"[Job Mode] job_id={args.job_id}")
        print(f"[Job Mode] DATA_DIR={DATA_DIR}")
        print(f"[Job Mode] RESULT_DIR={RESULT_DIR}")

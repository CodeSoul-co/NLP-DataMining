...
# ============================================================================
# Cloud-native path configuration - switch local/cloud via environment variables
# ============================================================================

# Base path - switched via environment variable THETA_BASE
# Local: /root/autodl-tmp
# Cloud (DLC): /mnt/data or OSS mount point
THETA_BASE = os.environ.get("THETA_BASE", "/root/autodl-tmp")
THETA_MODEL_SIZE = os.environ.get("THETA_MODEL_SIZE", "0.6B")

# Additional environment variable overrides for DLC compatibility
THETA_DATA_DIR = os.environ.get("THETA_DATA_DIR", None)  # Override data directory
THETA_RESULT_DIR = os.environ.get("THETA_RESULT_DIR", None)  # Override result directory
THETA_MODEL_DIR = os.environ.get("THETA_MODEL_DIR", None)  # Override model directory
THETA_SBERT_PATH = os.environ.get("THETA_SBERT_PATH", None)  # Override SBERT model path


def is_cloud() -> bool:
    """Check whether running in a cloud environment"""
    return THETA_BASE.startswith("oss://")


def join_path(*parts) -> str:
    """Join paths with compatibility for local filesystem and OSS"""
    if is_cloud():
        base = THETA_BASE.rstrip("/")
        path_parts = [str(p).strip("/") for p in parts if p]
        return base + "/" + "/".join(path_parts)
    else:
        return str(Path(THETA_BASE).joinpath(*[str(p) for p in parts]))


# Base paths - cloud-native paths with environment variable overrides
BASE_DIR = Path(THETA_BASE) if not is_cloud() else THETA_BASE
DATA_DIR = THETA_DATA_DIR if THETA_DATA_DIR else join_path("data")
EMBEDDING_DIR = join_path("embedding")
ETM_DIR = join_path("ETM") if not is_cloud() else "."
EMBEDDING_MODELS_DIR = THETA_MODEL_DIR if THETA_MODEL_DIR else join_path("embedding_models")
QWEN_MODEL_PATH = join_path("embedding_models", "qwen3_embedding_0.6B")
RESULT_DIR = THETA_RESULT_DIR if THETA_RESULT_DIR else join_path("result")

# SBERT model path with environment variable override
SBERT_MODEL_PATH = THETA_SBERT_PATH if THETA_SBERT_PATH else join_path("ETM", "model", "sbert", "sentence-transformers", "all-MiniLM-L6-v2")

# Embedding model paths and dimension mapping
QWEN_MODEL_PATHS = {
    '0.6B': join_path("embedding_models", "qwen3_embedding_0.6B"),
    '4B': join_path("embedding_models", "qwen3_embedding_4B"),
    '8B': join_path("embedding_models", "qwen3_embedding_8B"),
}
...
def get_qwen_model_path(model_size: str = None) -> str:
    """Get Qwen model path based on model_size"""
    size = model_size or THETA_MODEL_SIZE
    if size not in QWEN_MODEL_PATHS:
        raise ValueError(f"Unknown model size: {size}. Available: {list(QWEN_MODEL_PATHS.keys())}")
    return QWEN_MODEL_PATHS[size]


def get_sbert_model_path() -> str:
    """Get SBERT model path with environment variable support"""
    return SBERT_MODEL_PATH


def get_job_result_dir(job_id: Optional[str] = None, model_size: str = None) -> str:
    """
    Get result directory for a specific job.
    Supports multi-user isolation via job_id.
    
    Args:
        job_id: Unique job identifier for multi-user support
        model_size: Model size (e.g., '0.6B')
    
    Returns:
        Path to job-specific result directory
    """
    if job_id:
        return join_path("result", f"job_{job_id}")
    elif model_size:
        return join_path("result", model_size)
    else:
        return str(RESULT_DIR)


def get_job_data_dir(job_id: Optional[str] = None, dataset: str = None) -> str:
    """
    Get data directory for a specific job.
    Supports multi-user isolation via job_id.
    ...
    Returns:
        Path to job-specific data directory
    """
    if job_id:
        return join_path("data", f"job_{job_id}")
    elif dataset:
        return join_path("data", dataset)
    else:
        return str(DATA_DIR)


def get_embedding_dim(model_size: str = None) -> int:
    """Get embedding dimension based on model_size"""
    size = model_size or THETA_MODEL_SIZE
    return EMBEDDING_DIMS.get(size, 1024)
...
class EmbeddingConfig:
    ...
    output_dir: str = join_path("embedding", "outputs")
    batch_size: int = 64
    max_length: int = 512
...
# ============================================================================
# Parameter Constraints - used to validate parameters from frontend
# ============================================================================
...
PARAM_CONSTRAINTS = {
    "num_topics": {
        ...
        "description": "Number of topics. Recommend choosing based on dataset size"
    },
    "vocab_size": {
        "type": "int",
        "min": 1000,
...
def validate_params(params: dict) -> tuple:
    """
    Validate whether parameters are within constraints
    
    Args:
        params: Parameter dict, e.g. {"num_topics": 20, "vocab_size": 5000}
...
def get_param_options() -> dict:
    """
    Get option lists for all parameters (for frontend dropdown)

"""
Unified Trainer - Unified model trainer

Supports training of all topic models, including:
- ETM: requires doc_embeddings + word_embeddings + BOW
- CTM: requires doc_embeddings + BOW
- LDA: requires only BOW
...
class TopicModelTrainer:
    ...
    def train_neural_model(
        ...
        """
        Train neural network topic models (ETM, CTM, NeuralLDA, DTM)
        
        Args:
            bow_matrix: BOW matrix (num_docs, vocab_size)
            doc_embeddings: Document embedding (num_docs, embedding_dim)
            word_embeddings: Word embedding (vocab_size, embedding_dim)
            vocab: Vocabulary
            ...
            time_indices: Time indices (only for DTM)
...
def train_baseline_models(
    ...
    # Use config default if base_result_dir not specified
    if base_result_dir is None:
        import sys
        from pathlib import Path
        sys.path.insert(0, str(Path(__file__).parent.parent))
        from config import RESULT_DIR
        base_result_dir = os.path.join(str(RESULT_DIR), '0.6B')
    ...
    bow_path = os.path.join(bow_dir, 'bow_matrix.npz')
    if os.path.exists(bow_path):
        bow_matrix = sp.load_npz(bow_path).toarray()
    else:

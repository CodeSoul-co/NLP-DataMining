...
Strategy: Ensure balanced sampling of each dataset in every epoch
"""
...
class BalancedDatasetSampler(Sampler):
    ...
    Ensures balanced sample ratio of each dataset in every batch, avoiding large datasets "swallowing" small datasets.
    
    Strategy:
    1. Calculate sampling weight for each dataset (log scaling)
    2. Oversample small datasets, downsample large datasets
    3. Ensure all datasets are sufficiently sampled in each epoch
    """
    ...
    def __init__(
        ...
        Args:
            ...
            temperature: Temperature parameter, controls balancing degree (0=fully balanced, 1=original distribution)
            seed: Random seed
        """
    ...
    def _compute_sampling_strategy(self):
        ...
        if self.strategy == "oversample":
            # Oversample: all datasets sampled to the size of the largest dataset
            ...
            # Downsample: all datasets sampled to the size of the smallest dataset
            ...
            # Weighted sampling: use log scaling for balancing
            ...
            # temperature=0: fully balanced (all datasets have the same weight)
...
def create_balanced_dataloader(
    ...
    # Note: returns indices and dataset_ids
    # Actual text encoding needs to be done in training loop
    return {
        'texts': all_texts,

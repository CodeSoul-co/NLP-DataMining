...
def parse_args():
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default=f"{_THETA_MODEL_DIR}/qwen3_embedding_0.6B",
        help="Path to Qwen3-Embedding model (uses THETA_MODEL_DIR env var)"
    )
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default=f"{_THETA_BASE}/embedding/outputs",
        help="Output directory for embeddings (uses THETA_BASE env var)"
    )
    
    parser.add_argument(
        "--result_dir",
        type=str,
        default=_THETA_RESULT_DIR,
        help="Result directory (uses THETA_RESULT_DIR env var)"
    )

...
2. NeuralLDA: VAE-LDA implementation based on neural networks
...
class SklearnLDA(TraditionalTopicModel):
    ...
    2. No GPU required
    3. Suitable as Baseline comparison
    ...
    def __init__(
        ...
        n_jobs: int = -1,
        dev_mode: bool = False,
        # Following parameters for interface compatibility, not used by LDA
...
# ============================================================================
# Neural LDA (based on VAE)
# ============================================================================

class LDAEncoder(nn.Module):
    """
    LDA Encoder - Infer topic distribution from BOW
    
    Unlike ETM, LDA directly uses BOW as input (no pre-trained embedding required)
    """
...
class LDADecoder(nn.Module):
    """
    LDA Decoder - Generate word distribution from topic distribution
    """
...
class NeuralLDA(NeuralTopicModel):
    """
    Neural LDA - VAE-LDA implementation based on neural networks
    
    Main features:
    1. No pre-trained embedding required
    ...
    def forward(
        self,
        doc_embeddings: torch.Tensor,  # For interface compatibility, not used by LDA
        bow: torch.Tensor,
        **kwargs
        ...
        Args:
            doc_embeddings: Document embedding (not used by LDA, only for interface compatibility)
            bow: BOW matrix (batch, vocab_size)

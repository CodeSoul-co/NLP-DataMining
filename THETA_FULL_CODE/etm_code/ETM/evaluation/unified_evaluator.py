...
Unified topic model evaluator, supports all model types (THETA, LDA, ETM, CTM, DTM)
Uses the same 7 evaluation metrics:
1. PPL (Perplexity) - Perplexity
...
class UnifiedEvaluator:
    ...
    Supports evaluating all model types, generates evaluation results and visualizations in unified format
    """
    ...
    def compute_all_metrics(self, top_k: int = 10) -> Dict[str, Any]:
        ...
        Args:
            top_k: Take top-k words per topic for computation
    ...
    def _compute_irbo(self, top_k: int = 25, p: float = 0.9) -> float:
        """
        Compute Inverse Rank-Biased Overlap (iRBO)
        
        Args:
            top_k: Take top-k words per topic
            p: RBO parameter
        ...
        for i in range(num_topics):
            for j in range(i + 1, num_topics):
                # Compute RBO between two topics
                list1 = set(top_words[i])
                list2 = set(top_words[j])
                
                # Simplified RBO computation
    ...
    def generate_metrics_plots(self) -> List[str]:
        """
        Generate evaluation metrics visualization charts
...
def evaluate_model(
    ...
    """
    Convenience function: evaluate trained model
    
    Args:
        model_dir: Model directory (contains theta, beta, etc. files)

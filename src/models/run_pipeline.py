...
def find_workspace_dir(dataset: str, user_id: str = "default_user", workspace_dir: str = None) -> Path:
    ...
    # Use new three-level structure
    ws = get_workspace_path(user_id, dataset)
...
def run_baseline(model_name: str, args) -> Dict[str, Any]:
    ...
    # New directory structure: result/{user_id}/{dataset}/{model}/{timestamp}/
    base_dir = get_result_path(args.user_id, args.dataset)
    ...
    if not args.skip_viz:
        ...
        try:
            ...
            viz_dir = run_baseline_visualization(
                ...
                output_dir=str(model_dir / 'visualization'),
                language=args.language,
                dpi=300
            )

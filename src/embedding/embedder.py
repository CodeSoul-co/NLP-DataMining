...
class QwenEmbedder:
    ...
    def __init__(
        ...
        # Default model path from environment
        if model_path is None:
            import os
            model_path = os.environ.get('QWEN_MODEL_0_6B')
            if not model_path:
                # Try to get from registry
                try:
                    from registry import get_model_path
                    model_path = get_model_path('0.6B')
                except:
                    pass
            if not model_path:
                model_path = "qwen3_embedding_0.6B"  # Fallback to relative name
        """
    ...
    def embed_texts(
        ...
        if temp_dir is None:
            embedding_dir = os.environ.get('EMBEDDING_DIR', os.path.dirname(__file__))
            temp_dir = os.path.join(embedding_dir, 'outputs', f'.temp_{run_id}')
...
class EmbeddingManager:
    ...
    def __init__(
        ...
        if output_dir is None:
            embedding_dir = os.environ.get('EMBEDDING_DIR', os.path.dirname(__file__))
            output_dir = os.path.join(embedding_dir, 'outputs')

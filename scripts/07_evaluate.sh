...
if [ "$BASELINE" = true ] || [ "$MODEL" != "theta" ]; then
    EVAL_RESULT_DIR="$RESULT_DIR/baseline/$DATASET/vocab_$VOCAB_SIZE"
    MODEL_DIR="$EVAL_RESULT_DIR/$MODEL"
else
    EVAL_RESULT_DIR="$RESULT_DIR/$MODEL_SIZE/$DATASET/$MODE"
...
import sys
import os
sys.path.insert(0, os.environ.get('ETM_DIR', '.'))
import numpy as np
import json

...
class DocumentTopicUMAPVisualizer:
    ...
    def plot_document_clusters(
        ...
        # Get colormap - use HSV for vibrant, distinguishable colors
        if colormap == 'hsv' or colormap == 'rainbow':
            cmap = plt.cm.get_cmap(colormap)
        else:
            cmap = plt.cm.hsv  # Default to HSV for best color distinction
        
        # Get unique topics
        unique_topics = np.unique(doc_topics)
        n_unique = len(unique_topics)
        
        # Pre-compute colors using HSV for maximum distinction
        colors = [cmap(i / n_unique) for i in range(n_unique)]
